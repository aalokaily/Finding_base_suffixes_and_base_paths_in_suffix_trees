...
def Find_and_check_base_suffixes_using_non_trivial_algorithm2(tree):
    ...
    print ("Cost for sorting all SLS", "{:,}".format(cost))
            ...
            if not current_node.is_leaf():
                ...
                if hasattr(current_node, "SLS"):
                    for leaf_node in get_leaf_nodes(tree, current_node):
                        ...
                        if right_pos == len(current_node.SLS_sorted_by_leaf_index_under_ST):
                            right_pos = right_pos - 1
                        node_suffix_link_to_current_node = current_node.SLS_sorted_by_leaf_index_under_ST[right_pos][2]
